 ...
 *   admin_label = @Translation("Socializer Share Buttons"),
 * )
 */
 class SocializerShareBlock extends BlockBase implements ContainerFactoryPluginInterface {
  ...
  public function build() {
    ...
    foreach ($social_networks as $key => $label) {
      if ($label['share_enable'] == 1 && !empty($label['api_url'])) {
        $networks[$key]['text'] = $label['text'];
        $networks[$key]['api_url'] = $this->token->replace($label['api_url']);
    ...
    $build['socializer_share_block'] = array(
      '#theme' => 'social_network_share',
      '#elements' => $networks,
  ...
  public function blockSubmit($form, FormStateInterface $form_state) {
    $this->setConfigurationValue('class', $form_state->getValue('share_block_class'));
  }
...
public function defaultConfiguration() {
  ...
  return array(
    'class' => $default_config->get('display_settings.share_buttons.class')
  );
}
  ...
  protected function sortNetworks($element) {
    ...
    foreach ($element as $key => $weight_setting) {
      $weight[$key] = $weight_setting['share_order'];
    }
